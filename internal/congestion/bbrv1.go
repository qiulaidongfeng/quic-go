...
type BBRv1Sender struct {
	...
	latelybandwidth            [8]protocol.ByteCount
	initialMaxCongestionWindow protocol.ByteCount
	lastSendTime               monotime.Time
...
func NewBBRv1Sender(initialMaxDatagramSize, initialMaxCongestionWindow protocol.ByteCount) *BBRv1Sender {
	return &BBRv1Sender{
		state:                      STARTUP,
...
func (b *BBRv1Sender) HasPacingBudget(now monotime.Time) bool {
	...
	return b.bytesInFlight < protocol.ByteCount(float64(b.cwnd()))
}
...
func (b *BBRv1Sender) cwnd() protocol.ByteCount {
	...
	bdp := b.maxBandwidth * (protocol.ByteCount(b.minRTT()))
	return protocol.ByteCount(float64(bdp) * b.cwnd_gain)
}
...
func (b *BBRv1Sender) OnPacketSent(sentTime monotime.Time, bytesInFlight protocol.ByteCount, packetNumber protocol.PacketNumber, bytes protocol.ByteCount, isRetransmittable bool) {
	...
	b.nextSendTime = b.lastSendTime + monotime.Time(float64(bytes)/(b.pacing_gain*float64(b.maxBandwidth)))
}

func (b *BBRv1Sender) CanSend(bytesInFlight protocol.ByteCount) bool {
	now := monotime.Now()
	if now < b.nextSendTime {
		return false
	}
	return b.HasPacingBudget(now)
}
...
func (b *BBRv1Sender) OnPacketAcked(number protocol.PacketNumber, ackedBytes protocol.ByteCount, priorInFlight protocol.ByteCount, eventTime monotime.Time) {
	...
	delivery_rate := (b.delivered / max(1, protocol.ByteCount(eventTime-b.delivered_time)))
	if eventTime-b.round_start_time >= monotime.Time(minrtt) && b.state != PROBE_RTT {
		if b.inRecovery {
			b.exit_recover()
		}
		b.latelybandwidth[b.round%8] = delivery_rate
		b.round++
	...
	if b.state == DRAIN && priorInFlight < b.cwnd() {
		b.entry_PROBE_BW()
	}
	app_limited := priorInFlight < b.maxBandwidth*(protocol.ByteCount(b.minRTT()))
	if (delivery_rate > b.maxBandwidth || (!app_limited && delivery_rate > 0)) && b.state != PROBE_RTT && b.state != DRAIN && !b.inRecovery {
